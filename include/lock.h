 ...
 */
struct Mutex * Mutex_Create(int mode);

/** Lock a mutex
 * @param mutex The mutex to lock.
 * @return true on success false on failure.
 */
bool Mutex_Lock(struct Mutex *mutex);
 ...
 */
struct Semaphore * Semaphore_Create(bool shared, unsigned int value);

/** Post to a semaphore
 * @param sem The semaphore to post to.
 * @return true on success false on failure.
 */
bool Semaphore_Post(struct Semaphore *sem);
/** Wait for a slot on the semaphore
 * @param sem The semaphore to wait on.
 * @return true on success false on failure.
 */
bool Semaphore_Wait(struct Semaphore *sem);
bool Semaphore_TimedWait(struct Semaphore *sem);
